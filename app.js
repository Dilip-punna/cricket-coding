...
app.get("/players/", async (request, response) => {
  ...
  response.send(
    playerArray.map((eachplayer) => convertDBobjectToResponse(eachplayer))
  );
});
...
app.post("/players/", async (request, response) => {
  const playeradd = request.body;
  const { playerName, jerseyNumber, role } = playeradd;
  const Query = `
     INSERT INTO 
     cricket_team(
      player_name,
...
app.put("/players/:playerId/", async (response, request) => {
  const play = request.body;
  const { playerName, jerseyNumber, role } = play;
  const { playerId } = request.params;
